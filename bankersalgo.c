...
{ 
    ...
    { 
        for (int i = 0; i < n; i++) 
        { 
            if (remaining[i].A <= avA && remaining[i].B <= avB && remaining[i].C <= 
avC && iscompleted[i] == false) 
            { 
                avA += allocated[i].A; 
                avB += allocated[i].B; 
                avC += allocated[i].C; 
...
allocated[i].B, allocated[i].C, max_need[i].A, max_need[i].B, max_need[i].C, 
remaining[i].A, remaining[i].B, remaining[i].C, available[i].A, available[i].B, 
available[i].C); 
    } 
    return 0;
}